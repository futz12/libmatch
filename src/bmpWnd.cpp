...
LRESULT TextWindow::windowProc(UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
	case WM_TIMER:
		if (wParam == 0) {
...
TextWindow::TextWindow(LPCSTR text, int fontSize)
	: m_text(text)
{
	LOGFONTA lf;
...
{
	float scaleFactor = getDpiFactor();
	if (!create(nullptr, WS_POPUP, 0, 0, 500 * scaleFactor, 300 * scaleFactor, nullptr, WS_EX_TOPMOST)) {
		std::cerr << "喜报: 你的免责声明炸了\n";
		return;
	}
	centerWindow();
